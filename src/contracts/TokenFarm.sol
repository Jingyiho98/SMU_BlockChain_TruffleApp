...
contract TokenFarm {
    ...
    function P2P_Xfers(uint256 _amount,  string memory _type , string memory _typeto) public {
        //error handling
        require(_amount > 0, "amount cannot be 0");
        //string memory recieverRAW = '0x62D7a3B65bC34Fc623552119865F90939b550054';
        address reciever = parseAddr(_typeto);

        // deposit
        if (keccak256(abi.encodePacked(_type)) == keccak256(abi.encodePacked("dai"))){
            daiToken.transferFrom(msg.sender,address(reciever), _amount);
        }
        if (keccak256(abi.encodePacked(_type)) == keccak256(abi.encodePacked("dapp"))){
            dappToken.transferFrom(msg.sender, address(reciever), _amount);
        }
        if (keccak256(abi.encodePacked(_type)) == keccak256(abi.encodePacked("bdtf"))){
            bdtfToken.transferFrom(msg.sender, address(reciever), _amount);
        }
    }
